...
class StdOutListener(StreamListener):
    ...
    def on_status(self, data):
        ...
        self.db.sports.insert({"name":data.user.name,"text":data.text,"created_at":data.created_at,"positive_score:":analysis.sentiment.p_pos*100,"negative_score:":analysis.sentiment.p_neg*100})
        return True
...
if __name__ == '__main__':
    ...
    stream.filter(languages=["en"],track=['sports','cricket','football','basketball','dhoni','virat kohli','sachin','olympics','worldcup','FIFA','games','volleyball','stadium'])
